import logo from '../assets/public/logo.svg';
import anime from '../assets/public/anime.png';

function Hero() {
  return (
    <div className="bg-hero bg-center bg-cover bg-no-repeat sm:p-16 py-16 px-8 flex justify-center lg:items-center max-lg:flex-col w-full sm:gap-16 gap-0">
      <div className="flex-1 flex flex-col gap-10">
        <img
          src={logo}
          alt="logo"
          width={101}
          height={96}
          className="object-contain"
        />
        <div className="sm:text-6xl text-5xl text-white lg:max-w-lg font-bold leading-[120%]">
          Explore The <span className="red-gradient">Diverse Realms</span> of
          Anime Magic
        </div>
      </div>
      <div className="lg:flex-1 relative w-full max-h-[50vh] justify-center">
        <img
          src={anime}
          alt="anime"
          className="object-contain w-full  h-full"
        />
      </div>
    </div>
  );
}

export default Hero;
